...
int main(int c, char *argv[]) {
  ...
  sem_init(&queue0.itemsSem, 0, queue0.count);
  sem_init(&queue1.spaceSem, 0, queueSize);
  sem_init(&queue1.itemsSem, 0, queue1.count);
  sem_init(&queue2.spaceSem, 0, queueSize);
  sem_init(&queue2.itemsSem, 0, queue2.count);  

  // Launch the threads
  thread intsT, primesT, foursT;
  intsT = thread(addInts, N);
  primesT = thread(findPrimes, 0);
  foursT = thread(findFourSquares, 0);
